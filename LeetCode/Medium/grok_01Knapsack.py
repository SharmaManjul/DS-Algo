...
def solve_knapsack(profits, weights, capacity):
    ...
    # 2-Dimensional list to store the table with x axis as the capacities starting from 0 and y axis as the indexes
    # starting at 0.
    knapsack_list = [[x for x in range(capacity+1)] for y in range(2)]
    ...
    for i in range(index_len):
        for c in range(capacity+1):
            ...
            if c == 0:
                knapsack_list[i%2][c] = 0

            if i == 0 and weights[i%2] <= c:
                knapsack_list[i%2][c] = profits[i]

            if weights[i] <= c:
                profit1 = profits[i] + knapsack_list[(i-1)%2][c-weights[i]]

            profit2 = knapsack_list[(i-1)%2][c]

            knapsack_list[i%2][c] = max(profit1, profit2)

    return knapsack_list[1][capacity]
